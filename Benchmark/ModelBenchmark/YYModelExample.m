...
+ (NSDictionary *)modelContainerPropertyGenericClass {
    return @{@"photos" : YYPhoto.class,
             ...
             @"likedUserIds" : NSNumber.class};// lzy注171227：指定将数组中的元素，解析为某个类型。
}
@end
...
- (BOOL)modelCustomTransformFromDictionary:(NSDictionary *)dic {
    /* lzy注171228:
     unsignedLongLongValueForKey这个方法，来自NSDictionary+YYAdd。对这个分类方法的使用场景，不是很理解。
     在数据处理的这个情况，用到，这是非常好的。
     也解决了我的一个疑惑，有些字段关键逻辑依靠后台的某个字段，需要判断是否为空。YYModel解析空的默认规则需要熟悉。
     也可以在这个方法中自己处理。
     */
